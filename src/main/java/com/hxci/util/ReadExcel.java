...
public class ReadExcel {
     ...
     * @return
     */
    public List<StudentDetail> getExcelInfo(String fileName, MultipartFile Mfile) {
...
//        }
        ...
        //初始化客户信息的集合
        List<StudentDetail> customerList = new ArrayList<StudentDetail>();
        //初始化输入流
        InputStream is = null;
        try {
            //验证文件名是否合格
...
//            is = new FileInputStream(file1);
     ...
     * @throws IOException
     */
    public List<StudentDetail> getExcelInfo(InputStream is, boolean isExcel2003) {
        List<StudentDetail> customerList = null;
        try {
            /** 根据版本选择创建Workbook的方式 */
            Workbook wb = null;
            //当excel是2003时
     ...
     * @return
     */
    private List<StudentDetail> readExcelValue(Workbook wb) {
        //得到第一个shell
        ...
        List<StudentDetail> customerList = new ArrayList<StudentDetail>();
        StudentDetail customer;
        //循环Excel行数,从第二行开始。标题不入库
        for (int r = 1; r < totalRows; r++) {
            Row row = sheet.getRow(r);
            if (row == null) continue;
            customer = new StudentDetail();
